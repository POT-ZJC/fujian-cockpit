    ...
    <div class="div-table">
      <el-scrollbar style="height:100%" class="scrollbar">
        ...
        <el-row class="tr" v-for="(item, index) in tableData" :key="index">
          <el-col class="td title  " style='color: rgb(58, 143, 246);'  >{{ filterStr(item.title) }}</el-col>
          <el-col class="td field tc">{{
...
<script>
import moduleWrapper from "@/views/cockpit/cockpit-bridge-v1/components/ui/module-wrapper"; //bridgeTechnicalStatus
...
import { ajaxFunObj } from "@/views/cockpit/cockpit-bridge-v1/commonFun";
export default {
  components: { moduleWrapper, cockpitSelect },
  computed: {
    bridgeTechnicalStatus() {
      return bridgeStore.bridgeTechnicalStatus;
    },
